...
def _extract_response_text(payload: dict) -> str:
    # OpenAI/MLflow Responses-style output
    pieces = []
    ...
    # Defensive fallbacks for alternate response envelopes
...
def generate_answer(prompt: str) -> tuple[str, dict]:
    ...
    if r.status_code != 200:
        raise RuntimeError(
            f"MODEL_CALL_FAILED status={r.status_code} body={r.text[:2000]}"
        )
...
@mlflow.trace(name="project14_dev_rag", span_type="CHAIN")
...
if __name__ == "__main__":
    ...
    out_file = out_dir / "rag_dev_e2e_result.json"
    ...
    for row in result["retrieved"]:
        print(
            f"  rank={row['rank']} chunk_id={row['chunk_id']} "
            f"document_id={row['document_id']} category={row['category']} "
            f"score={row['score']}"
        )
    usage = result["model_meta"].get("usage") or {}
    if usage:
        print("USAGE=" + json.dumps(usage, sort_keys=True))
    print(f"RESULT_JSON={out_file}")
    print("RAG_DEV_E2E=PASS")
