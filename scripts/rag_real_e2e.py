...
def build_grounded_prompt(question: str, rows: list[dict]) -> str:
    ...
    for i, row in enumerate(rows, start=1):
        blocks.append(
            ...
            f"source={row.get('source')} chunk_position={row.get('chunk_position')}\n"
...
@mlflow.trace(name="project14_real_rag_final_only", span_type="CHAIN")
def rag(question: str) -> dict:
    mlflow.update_current_trace(tags={
        "project": "project14",
        "phase": "real_rag_e2e_final_only",
    ...
    return {
        ...
        "retrieved": [{
            ...
            "chunk_position": row.get("chunk_position"),
            "score": row.get("score"),
        } for i, row in enumerate(rows, start=1)],
        "model": MODEL_SERVICE,
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--question",
        default="What is studied about massive photon-pair production at hadron colliders?",
    )
    args = ap.parse_args()
    ...
    out_file = out_dir / "rag_real_e2e_final_only_result.json"
    ...
    print("RAG_REAL_E2E_FINAL_ONLY=PASS")
