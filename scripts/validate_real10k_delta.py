from __future__ import annotations

import os
from databricks import sql
from dotenv import load_dotenv

load_dotenv()

host = os.environ["DATABRICKS_SERVER_HOSTNAME"].replace("https://", "").rstrip("/")
http_path = os.environ["DATABRICKS_HTTP_PATH"]
token = os.environ["DATABRICKS_TOKEN"]
catalog = os.environ.get("DATABRICKS_CATALOG", "p14_rag_llmops")
schema = os.environ.get("DATABRICKS_SCHEMA", "default")

with sql.connect(
    server_hostname=host,
    http_path=http_path,
    access_token=token,
) as conn:
    with conn.cursor() as cur:
        cur.execute(f"USE CATALOG `{catalog}`")
        cur.execute(f"USE SCHEMA `{schema}`")

        cur.execute("""
        SELECT
          (SELECT count(*) FROM scientific_documents_prod) AS docs,
          (SELECT count(*) FROM scientific_chunks_prod) AS chunks,
          (SELECT count(*) FROM scientific_documents_prod d
             LEFT ANTI JOIN scientific_chunks_prod c
             ON d.document_id = c.document_id) AS docs_without_chunks,
          (SELECT count(*) FROM scientific_chunks_prod
             WHERE chunk_text IS NULL OR trim(chunk_text) = '') AS empty_chunks,
          (SELECT count(*) FROM scientific_chunks_prod
             WHERE token_count > 192) AS oversized_chunks,
          (SELECT count(*) FROM (
             SELECT chunk_id
             FROM scientific_chunks_prod
             GROUP BY chunk_id
             HAVING count(*) > 1
          )) AS duplicate_chunk_ids
        """)
        row = cur.fetchone()

docs, chunks, missing, empty, oversized, duplicate_ids = row
passed = (
    docs == 10000
    and chunks >= docs
    and missing == 0
    and empty == 0
    and oversized == 0
    and duplicate_ids == 0
)

print(f"DOCUMENTS={docs}")
print(f"CHUNKS={chunks}")
print(f"DOCUMENTS_WITHOUT_CHUNKS={missing}")
print(f"EMPTY_CHUNKS={empty}")
print(f"OVERSIZED_CHUNKS={oversized}")
print(f"DUPLICATE_CHUNK_IDS={duplicate_ids}")
print("REAL10K_DELTA_VALIDATION=" + ("PASS" if passed else "FAIL"))
